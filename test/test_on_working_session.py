...
@pytest.fixture
def setup_simpletest_dir(tmpdir):
    
    dirA_path = str(tmpdir.join('dirA'))
    dirB_path = str(tmpdir.join('dirB'))
    
    # Copy the file structure
    shutil.copytree('test/simpletest_dirA/', dirA_path)
    shutil.copytree('test/simpletest_dirB/', dirB_path)
...
def test_simple_run_synchronizer_fullauto(setup_simpletest_dir):
    ws, dirA_path, dirB_path = setup_simpletest_dir
